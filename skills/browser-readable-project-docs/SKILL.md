...
## How to use
1. **At step 0** (already copied? skip — never overwrite a project's copy), copy the bundled `scripts/render_docs.py` into the project (e.g. `scripts/`), and create two data files:
   ```json
   // decisions.json
   ...
   `session-handoff`) read it on re-entry, so keep it truthful the moment a
   step starts or the human clears its gate; `plan.md` stays the contract and
